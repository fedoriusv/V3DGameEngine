    ...
    {
        {
            ...
            m_pipelineMSAA->setDepthCompareOp(CompareOperation::CompareOp_Less);
...

        {
            ...
            m_pipelineBackbuffer = cmdList.createObject<renderer::GraphicsPipelineState>(renderer::VertexInputAttribDescription(), m_programBackbuffer.get(), m_renderTargetBackbuffer.get());
...
void TextureRender::process(renderer::CommandList& cmdList, const std::vector<std::tuple<renderer::StreamBufferDescription, renderer::DrawProperties>>& props)
{
    if (m_enableMSAA)
    {
        cmdList.setViewport(core::Rect32(0, 0, m_renderTargetMSAA->getDimension().width, m_renderTargetMSAA->getDimension().height));
        cmdList.setScissor(core::Rect32(0, 0, m_renderTargetMSAA->getDimension().width, m_renderTargetMSAA->getDimension().height));
    ...
    if (m_enableMSAA)
    {
        cmdList.setViewport(core::Rect32(0, 0, m_renderTargetBackbuffer->getDimension().width, m_renderTargetBackbuffer->getDimension().height));
        cmdList.setScissor(core::Rect32(0, 0, m_renderTargetBackbuffer->getDimension().width, m_renderTargetBackbuffer->getDimension().height));
