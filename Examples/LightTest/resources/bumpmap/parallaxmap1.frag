...
void main() 
{
...
#if 1
    offsetCoords = parallaxMappingOcclusion(textureHeight, samplerFilter, offsetCoords, viewDir);
#else
    offsetCoords = parallaxMapping(textureHeight, samplerFilter, offsetCoords, viewDir);
#endif
    ...
    vec3 normalDiffuse = texture(sampler2D(textureNormal, samplerFilter), offsetCoords).xyz;
    vec3 normal = TBN * (normalDiffuse * 2.0 - 1.0);
    ...
    for (int l = 0; l < 1; ++l)
    {
        ...
        if (diffuseIntensive > 0.01)
        {
            ...
            //outFragColor += light.lights[l].specular * pow(max(dot(r, tangentViewDir), 0.01), specPower) * attenuation;
        }
    }
}