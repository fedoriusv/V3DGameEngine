...
ConstantBuffer<LIGHTS> light : register(b4);

struct PS_UBO 
{
    float4 viewPosition;
};
ConstantBuffer<PS_UBO> ps_ubo : register(b5);
...
float4 main_PS(PS_INPUT Input) : SV_TARGET0
{
    ...
    float4 colorTextureDiffuse = textureDiffuse.Sample(samplerColor, Input.UV);
    ...
    for (int l = 0; l < LIGHT_COUNT; ++l)
    {
        ...
        if (diffuseIntensive > 0.01)
        {
            ...
            float4 colorTextureSpecular = textureSpecular.Sample(samplerColor, Input.UV);
