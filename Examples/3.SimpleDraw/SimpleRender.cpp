...
SimpleRender::SimpleRender(renderer::CommandList& cmdList, const renderer::VertexInputAttribDescription& desc, const std::vector<f32>& geomentry)
{
    ...
    //m_renderTarget->setColorTexture(0, cmdList.getBackbuffer(), RenderTargetLoadOp::LoadOp_Clear, RenderTargetStoreOp::StoreOp_Store, core::Vector4D(0.5f));
    ...
    m_pipeline->setDepthCompareOp(CompareOperation::CompareOp_Greater);
...
void SimpleRender::render(renderer::CommandList& cmdList)
{
    ...
    SimpleRender::update(core::Vector3D(0.0f, 0.0f, -1.0f), core::Vector3D(0.0f));
    cmdList.draw(renderer::StreamBufferDescription(m_vetexBuffer, 0), 0, 3, 1);

    SimpleRender::update(core::Vector3D(0.0f, 0.5f, -1.0f), core::Vector3D(0.0f, 0.0f, 90.0f));
    cmdList.draw(renderer::StreamBufferDescription(m_vetexBuffer, 0), 0, 3, 1);
}
