...
static const std::map<std::string, renderer::ShaderType> k_SPIRV_ExtensionList =
{
    //glsl
...
Resource* ShaderSpirVDecoder::decode(const stream::Stream* stream, const Policy* policy, u32 flags, const std::string& name) const
{
    ...
    if (shaderPolicy->_content == renderer::ShaderContent::Source)
    {
        ...
        {
            LOG_ERROR("ShaderSpirVDecoder::decode: %s shader [%s], compile error %s", stringType.c_str(), name.c_str(), getCompileStatusError(status).c_str());
            if (result.GetNumErrors() == 0 && result.GetErrorMessage().empty())
            {
                return nullptr;
            }
        }
        ...
        {
            LOG_ERROR("ShaderSpirVDecoder::decode: %s shader [%s] shader error messages:\n%s", stringType.c_str(), name.c_str(), result.GetErrorMessage().c_str());
            return nullptr;
        }

        if (result.GetNumWarnings() > 0)
        {
            LOG_WARNING("ShaderSpirVDecoder::decode: %s shader [%s] shader warnings messages:\n%s", stringType.c_str(), name.c_str(), result.GetErrorMessage().c_str());
        }
        ...
        {
            LOG_ERROR("ShaderSpirVDecoder::decode: %s shader [%s] internal error :\n%s", stringType.c_str(), name.c_str(), result.GetErrorMessage().c_str());
            return nullptr;
        }
