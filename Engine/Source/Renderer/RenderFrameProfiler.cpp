...
RenderFrameProfiler::StackProfiler::StackProfiler(RenderFrameProfiler* profiler, u32 slot, FrameCounter counter) noexcept
    : m_profiler(profiler)
    , m_counter(counter)
    , m_slot(slot)
{
    m_profiler->start(m_slot, m_counter);
}

RenderFrameProfiler::StackProfiler::~StackProfiler()
{
    m_profiler->stop(m_slot, m_counter);
}

RenderFrameProfiler::RenderFrameProfiler(u32 slots, const std::vector<FrameCounter>& activeTimers, const std::vector<FrameCounter>& activeCounters) noexcept
    : m_counterFPS(0)
    , m_frameTime(0.f)
{
    ...
    m_metrics.resize(slots);

    for (u32 slot = 0; slot < slots; ++slot)
    {
        for (u32 index = 0; index < m_metrics[slot].size(); ++index)
        {
            Metric& metric = m_metrics[slot][index];
            metric.reset();
...
void RenderFrameProfiler::start(u32 slot, FrameCounter counter)
{
    m_active = true;

    Metric& metric = m_metrics[slot][toEnumType(counter)];
...
void RenderFrameProfiler::stop(u32 slot, FrameCounter counter)
{
    m_active = false;
...
void RenderFrameProfiler::collect()
{
    ...
    u32 indexCounters = 0;
    u32 indexTimers = 0;
    for (u32 slot = 0; slot < m_metrics.size(); ++slot)
    {
        for (u32 index = 0; index < m_metrics[slot].size(); ++index)
        {
            Metric& metric = m_metrics[slot][index];
