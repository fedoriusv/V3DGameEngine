...
namespace renderer
{
    ShaderProgram::ShaderProgram(Device* device, const VertexShader* vertex) noexcept
        : m_device(device)
    {
        m_shaders.fill(nullptr);
        memset(m_resourceSlots, k_invalidBinding, sizeof(m_resourceSlots));

        ASSERT(vertex, "must be valid");
        ...
        const auto& mappedResources = m_shaders[toEnumType(ShaderType::Vertex)]->getMappingResources();
